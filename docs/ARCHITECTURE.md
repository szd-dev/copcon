...
```
React State (实时)  →  Ringbuf (缓冲窗口 1024)  →  PostgreSQL / SQLite (增量 checkpoint)
```
...
```
React State (立即)  →  Ringbuf (缓冲)  →  PostgreSQL / SQLite (持久化)
─────────────────      ───────────────     ─────────────────
...
| **SQLite** | 嵌入式数据库 | Sessions、Messages、Todos（零配置，开发/轻量部署） |
...
docker compose up -d
  ...
  └── copcon-server          (8080)  ← Go 后端（也可使用 SQLite 替代 PostgreSQL）
         ↑
    OPENAI_BASE_URL=https://api.openai.com/v1  ← 直连 OpenAI 兼容 API
```

Server 直接通过 `OPENAI_BASE_URL` 和 `OPENAI_API_KEY` 直连任意 OpenAI 兼容的 LLM 服务。
...
| **Three-Layer Data Safety** | React State → Ringbuf → PostgreSQL/SQLite，逐层降级不丢数据 |
