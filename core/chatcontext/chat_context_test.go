...
func TestRequestInput_Resolve(t *testing.T) {
	...
	req := iface.InputRequest{
		Type:    iface.InterruptApproval,
		Message: "Approve this action?",
	}

	var resp *iface.InputResponse
	var err error
	...
	assert.Equal(t, iface.InterruptApproval, pending[0].Type)
	assert.NotEmpty(t, pending[0].ID)

	resolveErr := c.ResolveInput(pending[0].ID, &iface.InputResponse{
		Action:  "approved",
...
func TestPendingInputs(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		c.RequestInput(iface.InputRequest{Type: iface.InterruptApproval, Message: "first"})
	}()
	go func() {
		defer wg.Done()
		c.RequestInput(iface.InputRequest{Type: iface.InterruptQuestion, Message: "second"})
	}()
	...
	types := map[iface.InterruptType]int{}
	for _, p := range pending {
		types[p.Type]++
	}
	assert.Equal(t, 1, types[iface.InterruptApproval])
	assert.Equal(t, 1, types[iface.InterruptQuestion])

	for _, p := range pending {
		err := c.ResolveInput(p.ID, &iface.InputResponse{Action: "ok"})
		require.NoError(t, err)
	}
...
func TestResolveInput_NotFound(t *testing.T) {
	...
	err := c.ResolveInput("nonexistent-id", &iface.InputResponse{Action: "approved"})
	assert.ErrorIs(t, err, iface.ErrInterruptNotFound)
}
...
func TestRequestInput_EmitsPartUpdate(t *testing.T) {
	...
	pending := c.PendingInputs()
	require.Len(t, pending, 1)
	c.ResolveInput(pending[0].ID, &iface.InputResponse{Action: "approved"})
...
func TestRequestInput_NoPartLocator(t *testing.T) {
	...
	req := iface.InputRequest{Type: iface.InterruptQuestion, Message: "test"}
	...
	pending := c.PendingInputs()
	require.Len(t, pending, 1)
	c.ResolveInput(pending[0].ID, &iface.InputResponse{Action: "answered"})
...
func TestChatContextInterfaceExtension(t *testing.T) {
	t.Run("Close_method_exists_on_interface", func(t *testing.T) {
		var ci iface.ChatContextInterface = NewChatContext(context.Background(), "s", "a")
		ci.Close()
	})
