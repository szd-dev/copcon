...
func toolCallsToStorage(tcs ToolCallsModel) []storage.ToolCall {
	if tcs == nil {
		return nil
	}
...
func toolCallsFromStorage(tcs []storage.ToolCall) ToolCallsModel {
	if tcs == nil {
		return nil
	}
	result := make(ToolCallsModel, len(tcs))
	for i, tc := range tcs {
		result[i] = ToolCallModel{
			ID:   tc.ID,
			Type: tc.Type,
