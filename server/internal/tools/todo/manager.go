...
type TodoOption func(*pgstore.Todo)

type TodoManager interface {
	CreateTodo(chatCtx iface.ChatContextInterface, content string, opts ...TodoOption) (*pgstore.Todo, error)
	GetTodo(chatCtx iface.ChatContextInterface, id string) (*pgstore.Todo, error)
	ListTodos(chatCtx iface.ChatContextInterface) ([]*pgstore.Todo, error)
	Update(chatCtx iface.ChatContextInterface, id string, updates map[string]any) (*pgstore.Todo, error)
	Delete(chatCtx iface.ChatContextInterface, id string) error
	Start(chatCtx iface.ChatContextInterface, id string) (*pgstore.Todo, error)
	Complete(chatCtx iface.ChatContextInterface, id string, result string) (*pgstore.Todo, error)
	Fail(chatCtx iface.ChatContextInterface, id string, reason string) (*pgstore.Todo, error)
	Block(chatCtx iface.ChatContextInterface, id string, reason string) (*pgstore.Todo, error)
	Unblock(chatCtx iface.ChatContextInterface, id string) (*pgstore.Todo, error)
	GetAvailableTodos(chatCtx iface.ChatContextInterface) ([]*pgstore.Todo, error)
}
...
func (m *todoManager) CreateTodo(chatCtx iface.ChatContextInterface, content string, opts ...TodoOption) (*pgstore.Todo, error) {
	sessionUUID, err := uuid.Parse(chatCtx.SessionID())
	if err != nil {
		return nil, fmt.Errorf("parse session id: %w", err)
	}

	var existing pgstore.Todo
	err = m.db.WithContext(chatCtx.Context()).
		Where("session_id = ? AND content = ? AND status != ?", sessionUUID, content, pgstore.TodoStatusCompleted).
	...
	todo := &pgstore.Todo{
		...
		Content:    content,
		Status:     pgstore.TodoStatusPending,
		RetryCount: 0,
		CreatedAt:  time.Now(),
		UpdatedAt:  time.Now(),
	}
...
func (m *todoManager) GetTodo(chatCtx iface.ChatContextInterface, id string) (*pgstore.Todo, error) {
	var todo pgstore.Todo
...
func (m *todoManager) ListTodos(chatCtx iface.ChatContextInterface) ([]*pgstore.Todo, error) {
...
func (m *todoManager) Update(chatCtx iface.ChatContextInterface, id string, updates map[string]any) (*pgstore.Todo, error) {
...
func (m *todoManager) Delete(chatCtx iface.ChatContextInterface, id string) error {
	result := m.db.WithContext(chatCtx.Context()).Delete(&pgstore.Todo{}, "id = ?", id)
	if result.Error != nil {
...
func (m *todoManager) Start(chatCtx iface.ChatContextInterface, id string) (*pgstore.Todo, error) {
	todo, err := m.GetTodo(chatCtx, id)
	if err != nil {
		return nil, err
	}

	currentStatus := pgstore.TodoStatus(todo.Status)
	if currentStatus != pgstore.TodoStatusPending && currentStatus != pgstore.TodoStatusBlocked {
...
func (m *todoManager) Complete(chatCtx iface.ChatContextInterface, id string, result string) (*pgstore.Todo, error) {
	if result == "" {
...
func (m *todoManager) Fail(chatCtx iface.ChatContextInterface, id string, reason string) (*pgstore.Todo, error) {
	todo, err := m.GetTodo(chatCtx, id)
	if err != nil {
		return nil, err
	}
	...
	updates := map[string]any{
		"status":      pgstore.TodoStatusFailed,
...
func (m *todoManager) Block(chatCtx iface.ChatContextInterface, id string, reason string) (*pgstore.Todo, error) {
	todo, err := m.GetTodo(chatCtx, id)
	if err != nil {
		return nil, err
	}
	...
	currentStatus := pgstore.TodoStatus(todo.Status)
	if currentStatus == pgstore.TodoStatusBlocked {
		return nil, ErrAlreadyBlocked
	}
	if currentStatus == pgstore.TodoStatusCompleted || currentStatus == pgstore.TodoStatusFailed {
		return nil, fmt.Errorf("%w: cannot block todo in terminal state %s", ErrTerminalState, todo.Status)
	}

	updates := map[string]any{
		"status":     pgstore.TodoStatusBlocked,
		"updated_at": time.Now(),
	}
...
func (m *todoManager) Unblock(chatCtx iface.ChatContextInterface, id string) (*pgstore.Todo, error) {
	todo, err := m.GetTodo(chatCtx, id)
	if err != nil {
		return nil, err
	}
...
func (m *todoManager) GetAvailableTodos(chatCtx iface.ChatContextInterface) ([]*pgstore.Todo, error) {
	var todos []*pgstore.Todo

	if err := m.db.WithContext(chatCtx.Context()).
		Where("session_id = ? AND status = ?", chatCtx.SessionID(), pgstore.TodoStatusPending).
		Find(&todos).Error; err != nil {
		return nil, fmt.Errorf("get available todos: %w", err)
	}

	var available []*pgstore.Todo
	for _, todo := range todos {
		...
		err := m.db.WithContext(chatCtx.Context()).
			Model(&pgstore.Todo{}).
			Where("id IN ? AND status = ?", todo.DependsOn, pgstore.TodoStatusCompleted).
...
func (m *todoManager) checkDependencies(chatCtx iface.ChatContextInterface, todo *pgstore.Todo) error {
	if len(todo.DependsOn) == 0 {
		return nil
	}
	...
	for _, depID := range todo.DependsOn {
		...
		if pgstore.TodoStatus(dep.Status) != pgstore.TodoStatusCompleted {
...
func (m *todoManager) validateNoCircularDeps(chatCtx iface.ChatContextInterface, todoID uuid.UUID, deps pgstore.UUIDArray) error {
...
func WithDependsOn(depIDs ...string) TodoOption {
	return func(t *pgstore.Todo) {
		deps := make(pgstore.UUIDArray, 0, len(depIDs))
...
func (m *todoManager) Create(ctx context.Context, todo *storage.Todo) (*storage.Todo, error) {
	...
	if model.Status == "" {
		model.Status = pgstore.TodoStatusPending
	}
...
func (m *todoManager) UpdateStatus(ctx context.Context, id uuid.UUID, status storage.TodoStatus) (*storage.Todo, error) {
	result := m.db.WithContext(ctx).
		...
		Where("id = ?", id).
		Updates(map[string]any{
			"status":     pgstore.TodoStatus(status),
			"updated_at": time.Now(),
		})
	if result.Error != nil {
...
func (m *todoManager) DeleteBySession(ctx context.Context, sessionID uuid.UUID) error {
	return m.db.WithContext(ctx).
		Where("session_id = ?", sessionID).
		Delete(&pgstore.Todo{}).Error
}

func todoFromStorage(t *storage.Todo) *pgstore.Todo {
	if t == nil {
		return nil
	}
	return &pgstore.Todo{
		...
		Status:      pgstore.TodoStatus(t.Status),
		DependsOn:   pgstore.UUIDArray(t.DependsOn),
...
func todoToStorage(t *pgstore.Todo) *storage.Todo {
	if t == nil {
		return nil
	}
	return &storage.Todo{
