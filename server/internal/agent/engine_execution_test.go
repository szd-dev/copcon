...
func (m *mockToolManagerWithTools) GetOpenAITools() []openai.ChatCompletionToolUnionParam {
...
func (m *mockOrderedContextManager) GetHistory(chatCtx iface.ChatContextInterface, limit int) ([]session.Message, error) {
	return nil, nil
}

func (m *mockOrderedContextManager) AddMessage(chatCtx iface.ChatContextInterface, msg *session.Message) error {
	m.mu.Lock()
	defer m.mu.Unlock()
