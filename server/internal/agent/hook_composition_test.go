...
func TestHookComposition_Engine(t *testing.T) {
	...
	chatCtxCreate := iface.NewChatContext(ctx, "", "composition-test")
	sess, err := sessionMgr.Create(chatCtxCreate, "Test Session", "composition-test")
...
func TestHookComposition_BackwardCompat(t *testing.T) {
	...
	sessionMgr := newMockSessionManager()
	ctx := context.Background()
	chatCtxCreate := iface.NewChatContext(ctx, "", "no-hooks-agent")
	sess, err := sessionMgr.Create(chatCtxCreate, "Test Session", "no-hooks-agent")
	require.NoError(t, err)
