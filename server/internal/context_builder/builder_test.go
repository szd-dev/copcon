...
func TestBuild_ContextCanceled(t *testing.T) {
	...
	cancel()
	// Build doesn't use context for anything currently (pure function),
	// but the interface accepts it for future extensions.
...
func TestConvertSessionToolCalls_Empty(t *testing.T) {
	result := ConvertSessionToolCalls(nil)
	assert.Nil(t, result)

	result = ConvertSessionToolCalls([]session.ToolCall{})
	assert.Nil(t, result)
}

func TestConvertSessionToolCalls_Conversion(t *testing.T) {
	input := []session.ToolCall{
		{ID: "tc1", Type: "function", Function: session.FunctionCall{Name: "bash", Arguments: `{"cmd":"ls"}`}},
		{ID: "tc2", Type: "function", Function: session.FunctionCall{Name: "python", Arguments: `{"code":"1+1"}`}},
	}
